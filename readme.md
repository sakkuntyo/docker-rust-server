...
[Rust Monitor 0.2.16 の ZIP をダウンロード](https://github.com/sakkuntyo/docker-rust-server/releases/download/1.1.22/RustMonitor-0.2.16-windows.zip)。「＋アイテム」からカテゴリ・検索・お気に入り付きの画像一覧を開き、選択したプレイヤーへ数量を指定してアイテムを付与できます。選択したプレイヤーのコンバットログを別窓で表示し、1分ごとに新しい記録を追記します。右側のSteam ID・本IPの横からSteamプロフィールとIPinfoを開けます。Steam ID・本IP・座標などのプレイヤー情報、チャット、サーバー一覧の表示文字を選択してコピーできます。所持品は公式の標準アイコンを使ったスロット表示で、数量・耐久ゲージと、ホバー時の詳細を確認できます。プレイヤー情報を折りたたみ、アイテム一覧の表示領域を広げられます。一覧のスクロールバーは非表示で、ホイールで上下に移動できます。サーバーチャットも独立した別窓で開き、5秒ごとの更新とSERVER名義の発言を利用できます。「＋アイテム」は従来の最小サイズ1040×700で開きます。Windows 10/11 と .NET 10 Desktop Runtime が必要です。
