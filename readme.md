...
[Rust Monitor 0.2.7 の ZIP をダウンロード](https://github.com/sakkuntyo/docker-rust-server/releases/download/1.1.13/RustMonitor-0.2.7-windows.zip)。所持品は公式の標準アイコンを使ったスロット表示で、数量・耐久ゲージと、ホバー時の詳細を確認できます。右側の所持品エリアを広げ、ウィンドウ幅に応じて調整します。Steam IDとconntrackで照合した本IP、折りたためるサーバーチャット、SERVER名義の発言にも対応しています。Windows 10/11 と .NET 10 Desktop Runtime が必要です。
